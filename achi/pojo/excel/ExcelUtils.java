...
import com.deta.common.utils.proxy.HttpsUtils;
...
public class ExcelUtils {
    ...
    public static <T> List<T> readFile(File file, Class<T> clazz) throws IOException, InstantiationException, IllegalAccessException {
        JSONArray array = readFile(file);
        return getBeanList(array, clazz);
    }

    public static <T> List<T> readMultipartFile(MultipartFile mFile, Class<T> clazz) throws IOException, InstantiationException, IllegalAccessException {
    ...
    private static <T> List<T> getBeanList(JSONArray array, Class<T> clazz) throws InstantiationException, IllegalAccessException {
     ...
     * 获取每个对象的数据
     */
    private static <T> T getBean(Class<T> c, JSONObject obj, Map<Integer, String> uniqueMap) throws InstantiationException, IllegalAccessException {
        T t = c.newInstance();
        ...
        for (Field field : fields) {
            ...
            if (field.getName().equals(ROW_NUM)) {
                ...
                ReflectionUtils.setField(field,t,rowNum);
                continue;
            }
            // 是否需要设置异常信息
            ...
            if (field.getName().equals(ROW_DATA)) {
                ReflectionUtils.makeAccessible(field);
                ReflectionUtils.setField(field,t,obj.toString());
                continue;
            }
            // 设置对应属性值
        ...
        for (Field field : fields) {
            if (field.getName().equals(ROW_TIPS)) {
                ReflectionUtils.makeAccessible(field);
                ReflectionUtils.setField(field,t,sb.toString());
            }
        }
        return t;
    }
    ...
    private static <T> void setFieldValue(T t, Field field, JSONObject obj, StringBuilder uniqueBuilder, List<String> errMsgList) {
        ...
        try {
            if ("String".equalsIgnoreCase(fieldClassName)) {
                ...
                ReflectionUtils.setField(field,t,Boolean.valueOf(val));
            } else if ("int".equalsIgnoreCase(fieldClassName) || "Integer".equals(fieldClassName)) {
                try {
                    ReflectionUtils.setField(field,t,Integer.valueOf(val));
                } catch (NumberFormatException e) {
                    errMsgList.add(String.format("[%s]的值格式不正确(当前值为%s)", cname, val));
                }
            } else if ("double".equalsIgnoreCase(fieldClassName)) {
                ReflectionUtils.setField(field,t,Double.valueOf(val));
            } else if ("long".equalsIgnoreCase(fieldClassName)) {
                ReflectionUtils.setField(field,t,Long.valueOf(val));
            } else if ("BigDecimal".equalsIgnoreCase(fieldClassName)) {
                ReflectionUtils.setField(field,t,new BigDecimal(val));
            } else if ("Date".equalsIgnoreCase(fieldClassName)) {
                try {
                    ReflectionUtils.setField(field,t,new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(val));
                } catch (Exception e) {
                    ReflectionUtils.setField(field,t,new SimpleDateFormat("yyyy-MM-dd").parse(val));
                }
            }
        } catch (Exception e) {
